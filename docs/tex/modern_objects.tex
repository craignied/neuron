...
\texttt{AsyncJob} owns the lifecycle every long GUI operation shares: training,
...
is supplied by the caller, so the class speaks no wire format.

\subsection{procguard}
\index{procguard}
\index{procguard!fatal diagnostic}

\texttt{procguard::run(body, err, program)} is the command-line program's
