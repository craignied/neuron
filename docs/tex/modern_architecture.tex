...
The retired interactive menus remain available for compatibility, scripted
legacy operation, and regression testing, but receive no further feature work.
The REST API is authoritative for new interactive capabilities. Reproducible
command-line runs feed a
...
The REST contract is the implementation surface for new work, with a GUI
control added when a capability needs a human-facing control. The maintained
map is \texttt{docs/gui\_cli\_parity.md}; it preserves the completed legacy
coverage audit and records newer REST/GUI capabilities. No new feature receives
a menu equivalent.
...
and \texttt{BareProp} share their state, training, and forward equations
through \texttt{OneHiddenNet} and remain distinct concrete types; only
the biased model establishes a bias slot.
...
The published saliency idea, neuron's validation-driven adaptation, and the
complete object contract are explained in section~\ref{obd_objects}.
...
current model.
Section~\ref{crossval_objects} explains why selection must be nested inside each
assessment fold and cites the supporting model-selection evidence.
...
Section~\ref{auc_estimators} derives the distinction, defines the paired
contrast, and cites both estimators' primary papers.
...
Local server, REST handlers, and browser interface. \\ \hline
\texttt{asyncjob.*},
\texttt{procguard.*} &
The asynchronous-job lifecycle every long operation shares, and the
command-line program's top-level exception boundary. \\ \hline
\texttt{split.*} &
...
loop. \\ \hline
\texttt{onehidden.*} &
State, training, and forward equations shared by the two
